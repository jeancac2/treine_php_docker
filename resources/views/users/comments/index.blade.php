...
<h1 class="text-2xl font-semibold leading-tigh py-2">
    Comentários do Usuários: {{ $user->name }}
     <a href="{{ route('comments.create', $user->id) }}" class="bg-blue-900 rounded-full text-white px-4 py-2 text-sm">  +  </a>
</h1>

<form action="{{ route('comments.index', $user->id) }}" method="get" class="py-5">
    <input type="search" name="search" placeholder="Pesquisar" class="md:w-1/6 bg-gray-200 appearence-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500">
...
<table class="min-w-full leading-normal shadow-md rounded-lg overflow-hidden">
    ...
    <tr>
        ...
        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
            <a href="{{ route('comments.edit', ['user'=> $user->id, 'id'=>$comment->id]) }}" class="bg-green-200 rounded-full py-2 px-6 font-semibold">Editar</a>
        </td>
    </tr>
    @endforeach
      </tbody>
